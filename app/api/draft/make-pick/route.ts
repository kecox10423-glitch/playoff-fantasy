...
function buildRosterEmailHtml(
  teamName: string, leagueName: string, leagueUrl: string,
    ...
    roster.filter(p => p.position === pos).map(player => `
      <tr>
        <td style="padding:10px 16px; border-bottom:1px solid #1f2937; vertical-align:middle;">
          <span style="display:inline-block;background:${positionColor(player.position)};color:#ffffff;font-size:10px;font-weight:800;padding:2px 6px;border-radius:4px;min-width:32px;text-align:center;">${player.position}</span>
        </td>
        <td style="padding:10px 16px; border-bottom:1px solid #1f2937; vertical-align:middle;">
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        <tr><td style="padding:0 0 24px 0;text-align:center;">
          ...
          <h1 style="color:#ffffff;font-size:28px;font-weight:900;margin:0 0 8px 0;">Your Draft Is In</h1>
          <p style="color:#9ca3af;font-size:14px;margin:0;"><strong style="color:#d1d5db;">${teamName}</strong> · ${leagueName}</p>
        </td></tr>
        <tr><td style="background:#1f2937;border-radius:12px;overflow:hidden;">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr style="background:#111827;">
              <td style="padding:10px 16px;"><span style="color:#6b7280;font-size:11px;font-weight:700;text-transform:uppercase;">POS</span></td>
              <td style="padding:10px 16px;"><span style="color:#6b7280;font-size:11px;font-weight:700;text-transform:uppercase;">Player</span></td>
              <td style="padding:10px 16px;text-align:right;"><span style="color:#6b7280;font-size:11px;font-weight:700;text-transform:uppercase;">Drafted</span></td>
            </tr>
            ${playerRows}
          </table>
        </td></tr>
        <tr><td style="padding:24px 0 0 0;text-align:center;">
          <a href="${leagueUrl}" style="display:inline-block;background:#15803d;color:#ffffff;font-weight:800;font-size:14px;padding:12px 28px;border-radius:8px;text-decoration:none;">View Your League →</a>
        </td></tr>
        <tr><td style="padding:24px 0 0 0;text-align:center;">
...
async function sendRosterEmails(leagueId: string) {
  try {
    const { data: league } = await supabaseAdmin
      .from("leagues").select("name, commissioner_user_id").eq("id", leagueId).single();
    if (!league) return;

    const { data: members } = await supabaseAdmin
      .from("league_members").select("user_id, team_name").eq("league_id", leagueId);
    if (!members?.length) return;

    const { data: picks } = await supabaseAdmin
      .from("draft_picks").select("user_id, player_id, round, pick_number")
      .eq("league_id", leagueId).order("pick_number");
    if (!picks?.length) return;

    const playerIds = picks.map(p => p.player_id);
    const { data: players } = await supabaseAdmin
      .from("players").select("id, name, position, nfl_teams(abbreviation, seed)").in("id", playerIds);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const playerIdInt = parseInt(String(playerId));
    if (isNaN(playerIdInt)) {
      return NextResponse.json({ error: `Invalid playerId: ${playerId}` }, { status: 400 });
    }

    const { data: existing } = await supabaseAdmin
      .from("draft_picks").select("id")
      .eq("league_id", leagueId).eq("player_id", playerIdInt).single();
    ...
    const { error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        player_id: playerIdInt, pick_number: pickNumber,
